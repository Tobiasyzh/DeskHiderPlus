...
; A classic 24x24 transparent ICO (BMP/DIB + AND mask) is generated in memory
; when needed. This avoids the black-square issue that PNG-compressed ICO overlays
; can cause on Windows 10 while keeping the compiled EXE self-contained.
...
HideShortcutArrowsAdmin()
{
    ...
    if (!WriteTransparentIcon(BLANK_ICON_PATH))
    {
        RestoreDefaultRegistryView()
        return 25
    }
...
WriteTransparentIcon(filePath)
{
    ; Exact layout of the tested 24x24, 32-bit transparent ICO:
    ; 6-byte ICONDIR + 16-byte ICONDIRENTRY + 40-byte BITMAPINFOHEADER
    ; + 2304-byte XOR bitmap + 96-byte AND mask = 2462 bytes.
    VarSetCapacity(iconData, 2462, 0)

    ; ICONDIR
    NumPut(1, iconData, 2, "UShort")       ; Type = icon
    NumPut(1, iconData, 4, "UShort")       ; Image count = 1

    ; ICONDIRENTRY
    NumPut(24, iconData, 6, "UChar")        ; Width
    NumPut(24, iconData, 7, "UChar")        ; Height
    NumPut(1, iconData, 10, "UShort")       ; Planes
    NumPut(32, iconData, 12, "UShort")      ; Bits per pixel
    NumPut(2440, iconData, 14, "UInt")      ; Bytes in image
    NumPut(22, iconData, 18, "UInt")        ; Image offset

    ; BITMAPINFOHEADER
    NumPut(40, iconData, 22, "UInt")        ; Header size
    NumPut(24, iconData, 26, "Int")         ; Width
    NumPut(48, iconData, 30, "Int")         ; Height = XOR + AND
    NumPut(1, iconData, 34, "UShort")       ; Planes
    NumPut(32, iconData, 36, "UShort")      ; Bits per pixel
    NumPut(2304, iconData, 42, "UInt")      ; XOR bitmap size

    ; XOR bitmap is already all-zero RGBA. The 1-bit AND mask uses
    ; 24 transparent bits plus 8 padding bits per row: FF FF FF 00.
    Loop, 24
    {
        maskOffset := 2366 + (A_Index - 1) * 4
        NumPut(0x00FFFFFF, iconData, maskOffset, "UInt")
    }
    ...
    written := file.RawWrite(iconData, 2462)
    file.Close()
